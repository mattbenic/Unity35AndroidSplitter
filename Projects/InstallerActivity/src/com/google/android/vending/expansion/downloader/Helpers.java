...
public class Helpers {
    ...
    static public boolean doesFileExist(Context c, String fileName, long fileSize,
    	...
    	String dfeTag = "doesFileExist";
        // the file may have been delivered by Market --- let's make sure
        // it's the size we expect
        File fileForNewFile = new File(Helpers.generateSaveFileName(c, fileName));
        Log.d(dfeTag, "****Checking for file: "+fileForNewFile.getAbsolutePath());
        if (fileForNewFile.exists()) {
        	Log.d(dfeTag, "****File exists, checking for filesize of "+fileSize);
            if (-1 == fileSize || fileForNewFile.length() == fileSize) {
            	Log.d(dfeTag, "****Filesize is correct, success");
                return true;
            }
            Log.d(dfeTag, "****File size mismatch: "+fileForNewFile.length());
            if (deleteFileOnMismatch) {
                // delete the file --- we won't be able to resume
                // because we cannot confirm the integrity of the file
                fileForNewFile.delete();
                Log.d(dfeTag, "****Deleting mismatched file");
            }
        }
        else
        {
        	Log.d(dfeTag, "****File DOES NOT exist");
        }
        return false;
    }
